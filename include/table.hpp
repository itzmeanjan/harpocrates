...
// https://github.com/haarcuba/cpp-text-table/tree/f217b3d; while fixing some
// bugs which popped up due to enabling `-Werror -Weverything`
//
// Thanks to https://github.com/haarcuba for creating this minimalistic
// header-only library for printing tabular data onto console !
...
public:
  ...
  int width(unsigned i) const { return static_cast<int>(_width[i]); }
...
private:
  ...
  {
    unsigned int _byteLength = static_cast<uint32_t>(s.length());
#ifdef TEXTTABLE_ENCODE_MULTIBYTE_STRINGS
...
inline std::ostream&
operator<<(std::ostream& stream, const TextTable& table)
{
  if (table.rows().empty()) {
    return stream;
  }
